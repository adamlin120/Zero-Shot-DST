...
def read_data(args, path_name, SLOTS, tokenizer, description, dataset=None):
    ...
    # dials = json.load(f)
    dials = dataset = load_dataset(
        "schema_guided_dstc8", split=(dataset if dataset != "dev" else "validation")+"[:128]"
    )
    ...
    for dial_dict in dials:
        dial_dict["dial_id"] = dial_dict["dialogue_id"]
        dial_dict["domains"] = dial_dict["services"][0]
        assert isinstance(dial_dict["dial_id"], str)
        assert isinstance(dial_dict["domains"], str)
        ...
        slot_values = []
        for frame in dial_dict["turns"]["frames"][::2]:
            slot_value = {}
            for state in frame["state"]:
                for name, values in zip(
                    state["slot_values"]["slot_name"],
                    state["slot_values"]["slot_value_list"],
                ):
                    value = values[0]
                    slot_value[f"{state['active_intent']}-{name}"] = value
            slot_values.append(slot_value)
        users = dial_dict["turns"]["utterance"][::2]
        systems = dial_dict["turns"]["utterance"][1::2]
        for turn_id, (slot_value, user, sys) in enumerate(
            zip(slot_values, users, systems)
        ):
            turn = {
                "system": sys,
                "user": user,
                "state": {"slot_values": slot_value},
            }
...
def prepare_data(args, tokenizer):
    ...
    ALL_SLOTS = list(description.keys())

    data_train, _ = read_data(
        args, path_train, ALL_SLOTS, tokenizer, description, "train"
    )
